...
const vpc = process.env.VPC || false;
const area = process.env.DATAAREA;
// 添加短域名处理
const areaname = vpc ? area + vpc : area;
console.log(areaname);
// const shortDomain = !process.env.ShortDomain;
const MQ_RABBIT_HOST = `http://v3.mq-rabbit.server.${area}.sky1088.com`;
const MAN_MASTER_HOST = `http://v3.man.server.${area}.sky1088.com`;
const MAN_REDIS_HOST = `http://v3.manager-redis.server.${areaname}.sky1088.com`;
const MAN_MSSQL_HOST = `http://v3.manager-mssql.server.${areaname}.sky1088.com`;
const MAN_MYSQL_HOST = `http://v3.manager-mysql.server.${areaname}.sky1088.com`;
const RES_REDIS_HOST = `http://v3.res-redis.server.${areaname}.sky1088.com`;
const RES_MASTER_HOST = `http://v3.res.server.${areaname}.sky1088.com`;
const RES_OTS_HOST = `http://v3.res-ots.server.${areaname}.sky1088.com`;
const WEBHOOK_HOST = `http://webhook.${area}.sky1088.com`;
const TERMINAL_HOST = `http://master.m2m.sky1088.com`;

console.log(MQ_RABBIT_HOST);
console.log(MAN_MASTER_HOST);
console.log(MAN_REDIS_HOST);
console.log(MAN_MSSQL_HOST);
console.log(MAN_MYSQL_HOST);
console.log(RES_REDIS_HOST);
console.log(RES_MASTER_HOST);
console.log(RES_OTS_HOST);
console.log(WEBHOOK_HOST);
console.log(TERMINAL_HOST);
...
let getHttpOptions = function (url, data) {
    return {
        ...
        method: "POST",
        json: true,
        headers: {'User-Agent': 'Data-Push HYZ hjjhvail@gmail.com'},
        body: data
    };
};
    ...
    let fn = (done, reject) => {
        request(url, {method: "GET", charset: "utf-8"}, function (err, res, body) {
            ...
            if (!res || res.statusCode > 204) {
                if (debug) console.log(`GET URL : ${url} : ${res.statusCode}`);
            }
        });
    };
    return new Promise(fn).catch(err => {
        console.log(`Error PromiseGet : ${url}`);
        console.log(err);
        throw err;
    })
};
...
router.PromisePost = (url, data, cb) => {
    ...
    return new Promise(fn).catch(err => {
        console.log(`Error PromisePost : ${url}`);
        console.log(err);
        throw err;
    })
};
...
router.PromisePut = (url, data) => {
    ...
    return new Promise(fn).catch(err => {
        console.log(`Error PromisePut : ${url}`);
        console.log(err);
        throw err;
    })
};
